...
class CampaignExportController extends Controller
{
    ...
    public function exportJson(Campaign $campaign)
    {
        ...
        $data = [
            ...
            'summary' => [
                'delivery_rate' => $campaign->sent_count > 0 ? round(($campaign->delivered_count / $campaign->sent_count) * 100, 2) : 0,
                'read_rate' => $campaign->delivered_count > 0 ? round(($campaign->read_count / $campaign->delivered_count) * 100, 2) : 0,
                'success_rate' => $campaign->total_numbers > 0 ? round(($campaign->delivered_count / $campaign->total_numbers) * 100, 2) : 0,
            ],
